...
<script>
	...
	function setEmail() {
		var mailAddress =  document.frm_mod_member.select_email2.value; 
		document.frm_mod_member.email2.value = mailAddress;
	}
		
	function fn_delete_member(memberId ,delYn) {
		var confirmLeaveMember = confirm("정말로 탈퇴하시겠습니까?");
		...
		<table class="table table-bordered table-hover">
				...
				<tr>
					...
					<td>
					   ...
					   <select name="select_email2" class="form-control"  onChange="setEmail()"  title="직접입력" style="display:inline; width:100px; padding:0">
						 <option value="" <c:if test="${email3 eq 'none'}"> selected</c:if>>직접입력</option>
						 <option value="gmail.com" <c:if test="${email3 eq 'gmail.com'}"> selected</c:if>>gmail.com</option>
						 <option value="naver.com" <c:if test="${email3 eq 'naver.com'}"> selected</c:if>>naver.com</option>
		...
		<p align="right">
			<input type="hidden" name="command"  value="modify_my_info" /> 
			  <c:if test="${memberInfo.delYn == 'N' }">
			    <input type="button" value="회원탈퇴" class="btn btn-primary btn-sm"  onClick="fn_delete_member('${memberInfo.memberId}','Y')">
			  </c:if>
		</p>
	</form>
</body>
